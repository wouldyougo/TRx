...
{
    public class CoverSpreadOnQuoteChange
    {
        ...
        public CoverSpreadOnQuoteChange(OrderBookContext quotesProvider,
        ...
        private void MakeBuySignals(double buySpreadPrice)
        {
            foreach (StrategyHeader s in this.leftLeg)
            {
                ...
                this.logger.Log(String.Format("{0:dd/MM/yyyy H:mm:ss.fff}, {1}, сигнал {2} на отправку заявки на покупку (cover) спреда {3}.", DateTime.Now, this.GetType().Name, signal.ToString(), s.ToString()));
        ...
        private bool LongPositionExists()
        {
