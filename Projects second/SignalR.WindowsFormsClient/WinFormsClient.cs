...
namespace WinFormsClient
{
    ...
    public partial class WinFormsClient : Form
    {
        ...
        private async void ConnectAsync()
        {
            ...
            HubProxy.On<string>("AddMessage1", (message) =>
                this.Invoke((Action)(() =>
                    RichTextBoxConsole.AppendText(String.Format("{0}" + Environment.NewLine, message))
                ))
            );

            HubProxy.On<string, string, string, string, string, string, string>("AddMessageBar", (Symbol, DateTime, Open, High, Low, Close, Volume) =>
                this.Invoke((Action)(() =>
                    RichTextBoxConsole.AppendText(String.Format("{0}: {1}  {2}  {3}  {4}  {5}  {6}" + Environment.NewLine, Symbol, DateTime, Open, High, Low, Close, Volume))
                ))
            );
